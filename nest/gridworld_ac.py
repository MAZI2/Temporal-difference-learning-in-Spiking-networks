...
class GridWorldAC(PongNet):
    ...
    def __init__(self, apply_noise=True, num_neurons=20):
        ...
        self.motor_noise_spikes = nest.Create("spike_generator", self.num_output_neurons, {"spike_times": self.noise_spike_times})

        nest.Connect(
            self.motor_noise_spikes,
            self.motor_neurons,
...
#        nest.Connect(self.supp_current, self.intermediate_motor2, syn_spec={"weight": 1000, "delay": 1})
    ...
    def load_saved_weights(self, connections_data):
        """
        Update existing NEST synaptic weights (input→motor and input→striatum)
        from a saved connections_data dict.
        """
        print("🔄 Loading pretrained weights into existing NEST connections...")

        # --- INPUT → MOTOR ---
        input_to_motor = connections_data.get("input_to_motor", None)
        if input_to_motor:
            sources = input_to_motor["source"]
            targets = input_to_motor["target"]
            ...
            print(f"✅ Updated {len(sources)} input→motor connections.")
            print(conns)

        # --- INPUT → STRIATUM ---
        input_to_striatum = connections_data.get("input_to_striatum", None)
        if input_to_striatum:
            sources = input_to_striatum["source"]
            targets = input_to_striatum["target"]
            weights = input_to_striatum["weight"]

            conns = nest.GetConnections(self.input_neurons, self.striatum)
            for conn in conns:
                src, tgt = conn.get(["source", "target"]).values()
                for s, t, w in zip(sources, targets, weights):
                    if s == src and t == tgt:
            ...
            print(f"✅ Updated {len(sources)} input→striatum connections.")
