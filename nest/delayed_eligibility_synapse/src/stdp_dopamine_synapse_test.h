...
#ifndef stdp_dopamine_synapse_test_H
#define stdp_dopamine_synapse_test_H
...
``stdp_dopamine_synapse_test`` is a connection to create synapses with
...
.. listexamples:: stdp_dopamine_synapse_test
 ...
 * connection.
 */
class STDPDopaTestCommonProperties : public CommonSynapseProperties
{
public:
  /**
   * Default constructor.
...
inline long
STDPDopaTestCommonProperties::get_vt_node_id() const
{
...
/**
 * Class representing an stdp_dopamine_synapse_test with homogeneous parameters,
 * i.e. parameters are the same for all synapses.
 */
void register_stdp_dopamine_synapse_test( const std::string& name );

template < typename targetidentifierT >
class stdp_dopamine_synapse_test : public Connection< targetidentifierT >
{

public:
  typedef STDPDopaTestCommonProperties CommonPropertiesType;
   ...
   */
  stdp_dopamine_synapse_test();

  /**
   * Copy constructor from a property object.
   * Needs to be defined properly in order for GenericConnector to work.
   */
  stdp_dopamine_synapse_test( const stdp_dopamine_synapse_test& ) = default;
  stdp_dopamine_synapse_test& operator=( const stdp_dopamine_synapse_test& ) = default;
   ...
   */
  bool send( Event& e, size_t t, const STDPDopaTestCommonProperties& cp );

  void trigger_update_weight( size_t t,
    const std::vector< spikecounter >& dopa_spikes,
    double t_trig,
    const STDPDopaTestCommonProperties& cp );
...
private:
  ...
  // index
  void update_dopamine_( const std::vector< spikecounter >& dopa_spikes, const STDPDopaTestCommonProperties& cp );

  void update_weight_( double c0, double n0, double minus_dt, const STDPDopaTestCommonProperties& cp );

  void process_dopa_spikes_( const std::vector< spikecounter >& dopa_spikes,
    double t0,
    double t1,
    const STDPDopaTestCommonProperties& cp );
  void facilitate_( double kplus, const STDPDopaTestCommonProperties& cp );
  void depress_( double kminus, const STDPDopaTestCommonProperties& cp );
...
template < typename targetidentifierT >
constexpr ConnectionModelProperties stdp_dopamine_synapse_test< targetidentifierT >::properties;

//
// Implementation of class stdp_dopamine_synapse_test.
//

template < typename targetidentifierT >
stdp_dopamine_synapse_test< targetidentifierT >::stdp_dopamine_synapse_test()
  : ConnectionBase()
  , weight_( 1.0 )
  , Kplus_( 0.0 )
  , c_( 0.0 )
...
template < typename targetidentifierT >
void
stdp_dopamine_synapse_test< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
stdp_dopamine_synapse_test< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
...
template < typename targetidentifierT >
void
stdp_dopamine_synapse_test< targetidentifierT >::check_synapse_params( const DictionaryDatum& syn_spec ) const
{
  ...
  if ( kernel().vp_manager.get_num_threads() > 1 )
  {
    if ( syn_spec->known( names::c ) )
    {
      throw NotImplemented(
        ...
        "of parameter c in stdp_dopamine_synapse_test. "
    ...
    if ( syn_spec->known( names::n ) )
    {
      throw NotImplemented(
        "For multi-threading Connect doesn't support the setting "
        "of parameter n in stdp_dopamine_synapse_test. "
        "Use SetDefaults() or CopyModel()." );
    }
  }
}
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::update_dopamine_( const std::vector< spikecounter >& dopa_spikes,
  const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::update_weight_( double c0,
  double n0,
  double minus_dt,
  const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::process_dopa_spikes_( const std::vector< spikecounter >& dopa_spikes,
  double t0,
  double t1,
  const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::facilitate_( double kplus, const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::depress_( double kminus, const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline bool
stdp_dopamine_synapse_test< targetidentifierT >::send( Event& e, size_t t, const STDPDopaTestCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
stdp_dopamine_synapse_test< targetidentifierT >::trigger_update_weight( size_t t,
  const std::vector< spikecounter >& dopa_spikes,
  const double t_trig,
  const STDPDopaTestCommonProperties& cp )
{
  ...
  std::cout << "ggg from" << t_last_update_ - dendritic_delay << "to" << t_trig - dendritic_delay << std::endl;
...
#endif // of #ifndef stdp_dopamine_synapse_test_H
