...
Nagrada, ki jo predstavlja aktivnost dopaminskih nevronov, bo vedno večja ali enaka $0$, kar pomeni, da se bodo sinapse skozi čas le krepile. Povezave, odgovorne za izbiro določene akcije v določenem stanju, morajo zato med seboj tekmovati za prevlado. To nam omogoča sled upravičenosti, ki mora biti za sinapse, ki vodijo do pravilne akcije dovolj različna, da bodo te napram ostalim povezavam dovolj okrepljene za prevlado. To pomeni, da morajo biti povezave, ki vodijo do prave akcije dovolj okrepljene, da bodo v prihodnje povzročile večjo verjetnost izbire pravilne akcije. Sled upravičenosti je višja za povezave z višjimi utežmi, saj take povezave povzročajo več impulzov postsinaptičnega nevrona kot posledica proženja presinaptičneag. Pri nižjih vrednostih uteži, dokler imajo vse povezave približno enako utež, dovolj veliko razliko v sledi upravičenosti dosežemo z varianco v frekvenci impulzov, ki povzroči, da nekatere sinapse po naključju prispevajo k akciji nekoliko bolj ali manj. 
To pomeni, da se vrednosti eligibility trace razlikujejo med sinapsami. Posledično lahko dopaminska modulacija (trening signal) posodobi sinapse različno in vzpostavi razlikovanje med njimi, tudi če imajo vse povezave enake začetne uteži. Pri nižjih vrednostih uteži, kjer deterministični prispevek sinaps še ni dominanten, naključna varianca impulzov služi kot mehanizem, da “razbije simetrijo” in omogoči, da se sledi upravičenosti razlikujejo, dokler uteži niso dovolj velike, da prevladujejo deterministični prispevki. Pogosto se razlikovanje med sinapsami doseže z dopuščanjem negativih nagrad, ki sinapse, ki so bile odgovorne za napačno izbrano akcijo negativno posodobijo. V našem sistemu se bomo negativnim nagradam, ki zahtevajo negativno koncentracijo dopamina izognili, saj v naravi negativna koncentracija dopamina ni mogoča.
Varianco v frekvenci impulzov dosežemo s Poissonovim šumom. V diferencialni enačbi, ki določa spreminjanje $V_m$ pri nižjih vrednostih uteži nad $I_{\text{syn}}$ prevladuje $I_e$ oziroma $I_{\text{Poisson}}$ v našem primeru. Tako ima, dokler ne prevladuje deterministični prispevek okrepljenih povezav, Poissonov šum večji vpliv na $V_m$ in je tako varianca v frekvenci impulzov večja. To služi tudi kot mehanizem za raziskovanje (\textit{exploration}), ki bo pri nižjih utežeh, ko imajo vse povezave približno enake uteži omogočil naključno izbiranje akcije. Tekom učenja, ko se sinapse okrepijo, bo agent akcije izbiral v glavnem glede na prevladujoče sinapse, kar je mehanizem za izkoriščanje (\textit{exploitation}). Naš sistem bo tako tekom učenja postopoma višal razmerje med izkoriščanje in raziskovanjem.
Za model nevrona bomo uporabili model nevrona z eksponentnim jedrom, saj Poissonov šum v tem primeru povzroči večjo varianco izhodnih nevronov kot model z alfa jedrom, kar smo pokazali v poglavju \ref{sec:izbira_modela}.
\\
\\
